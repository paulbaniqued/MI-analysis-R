...
ggplot() +
  ...
  geom_ribbon(data = cum_ave2, aes(t_epoch_t, ymin=CI_2_lower, ymax=CI_2_upper), fill="goldenrod3", alpha=0.4) +
  geom_ribbon(data = cum_ave1, aes(t_epoch_t, ymin=CI_1_lower, ymax=CI_1_upper), fill="blue", alpha=0.4) +
  ...
  ylim(-100, 100) +                                       # CHANGE THIS TO CHANGE Y AXIS
  ...
  labs(title = sprintf("Grand Average of ERD/ERS in %s and %s Across All Participants' %s Trials", channel1, channel2, trial_name), subtitle = expression(paste("Relative ", mu, " Power % Change at 9-11 Hz, (S-B)/B x100"))) +
  scale_colour_manual(name="Legend", values=c("blue", "goldenrod3")) +
  theme_cowplot(12)
