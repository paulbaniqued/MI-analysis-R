...
eeg_basecorr1 <- t_epoch$t_epoch_sig1 - baseline_ave1
eeg_basecorr1 <- data.frame(eeg_basecorr1)
eeg_basecorr2 <- t_epoch$t_epoch_sig2 - baseline_ave2
...
ggplot() +
  ...
  ylim(-120, 50) +
  xlab("seconds") + ylab("%ERD/ERS") + 
  labs(title = sprintf("Percent Change in ERD/ERS of %s and %s for 1 Participant Across All Days", channel1, channel2), subtitle = expression(paste("ALL RIGHT TRIALS. ", "Relative ", mu, " Power at 8-12 Hz, (B-S)/B x100"))) +
  scale_colour_manual(name="Legend", values=c("#E7B800", "#00AFBB")) +
  theme_cowplot(12)
