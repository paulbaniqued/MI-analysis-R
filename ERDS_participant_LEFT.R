...
folder_dir = "C:/Users/mnpdeb/AOMI/"
p_code <- readline(prompt = "Enter participant code (e.g. nn000pp): ")
...
filename <- paste(folder_dir, p_code, "_", day, ".edf", sep = "")
print(filename)
...
eeg_oc1 <- raw_eeg %>% 
  ...
  mutate(C3 = C3*1 - FC5*0 - C1*0 - CP5*0) %>% # DISABLE SURFACE LAPLACIAN
...
eeg_oc2 <- raw_eeg %>% 
  ...
  mutate(C4 = C4*1 - FC6*0 - C2*0 - CP6*0) %>% # DISABLE SURFACE LAPLACIAN
...
ggplot() +
  ...
  labs(title = sprintf("Percent Change in ERD/ERS of %s and %s Across All %s Trials", channel1, channel2, trial_name), subtitle = expression(paste("ALL LEFT TRIALS ", "Relative ", mu, " Power at 8-12 Hz, (B-S)/B x100"))) +
...
ggplot() +
  ...
  geom_smooth(data = N_ave, aes(t_epoch_t, N_ave1, colour=channel1), size= 1.5, alpha=0.0, span = 0.1) +
  xlim(-2, 8) +
  ylim(-120, 50) +
  xlab("seconds") + ylab("%ERD/ERS") + 
  labs(title = sprintf("Percent Change in ERD/ERS of %s and %s for 1 Participant Across All Days", channel1, channel2), subtitle = expression(paste("ALL LEFT TRIALS. ", "Relative ", mu, " Power at 8-12 Hz, (B-S)/B x100"))) +
  scale_colour_manual(name="Legend", values=c("#E7B800", "#00AFBB")) +
  theme_cowplot(12)

save_1 = paste(p_code, "_compiled_oc1.csv", sep = "")
write.table(cum_erds_1, file = save_1, sep = ",")

save_2 = paste(p_code, "_compiled_oc2.csv", sep = "")
write.table(cum_erds_2, file = save_2, sep = ",")
