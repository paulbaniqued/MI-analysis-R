...
ggplot() +
  geom_vline(aes(xintercept = 0, y = NULL, size = 0.5, alpha = 0.6), linetype = "dashed", show.legend = FALSE) +
  geom_smooth(data = N_ave, aes(t_epoch_t, N_ave1, colour="#00AFBB"), size= 1.5, alpha=0.0, span = 0.1) +
  geom_smooth(data = N_ave, aes(t_epoch_t, N_ave2, colour="#E7B800"), size= 1.5, alpha=0.0, span = 0.1) +
  xlim(-2, 8) +
  ylim(-120, 50) +
  xlab("seconds") + ylab("%ERD/ERS") + 
  labs(title = sprintf("Percent Change in ERD/ERS of %s and %s (1 Participant) Across All Days", channel1, channel2), subtitle = expression(paste("Relative ", mu, " Power at 8-12 Hz, (B-S)/B x100"))) +
