...
import_eeg <- read.edf("C:/Users/Paul/20200221125938_nk446we_Stream.edf", read.annotations = TRUE, header.only = FALSE)
...
for (i in 1:100)
{
  ...
  t_epoch_sig1[epoch_counter] <- colMeans(eeg_bp_use1[2:21])
  t_epoch_sig2[epoch_counter] <- colMeans(eeg_bp_use2)
  epoch_counter = epoch_counter + 1
}
t_epoch
...
baseline <- dplyr::filter(t_epoch, between(t_epoch_t, -3.5, 0))
...
ggplot() +
  geom_vline(aes(xintercept = 0, y = NULL, size = 0.25, alpha = 0.6), linetype = "dashed", show.legend = FALSE) +
  geom_rect(aes(xmin = -6, xmax = -3.5, ymin = -Inf, ymax = Inf), fill = 'red', alpha = 0.20) +
  geom_rect(aes(xmin = 6.5, xmax = 9, ymin = -Inf, ymax = Inf), fill = 'red', alpha = 0.20) +
  ...
  labs(title = sprintf("Percent Change in ERD/ERS of %s and %s Across All %s Trials", channel1, channel2, trial_name), subtitle = expression(paste("Relative ", mu, " Power at 9-11 Hz, (B-S)/B x100"))) +
